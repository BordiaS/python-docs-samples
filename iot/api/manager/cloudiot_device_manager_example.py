...
class DeviceRegistry(object):
  ...
  def __init__(self, project_id, registry_id, cloud_region,
               service_account_json, api_key, pubsub_topic):
    """Lookup or create a device registry for the given project."""
    self.parent = 'projects/{}/locations/{}'.format(project_id, cloud_region)
    ...
    # Lookup or create the device registry. Here we bind the registry to the
    # given Cloud Pub/Sub topic. All devices within a registry will have their
    # telemetry data published to this topic, using attributes to indicate which
    ...
    try:
      ...
      if e.resp.status == 409:
        ...
        print 'Registry', registry_id, 'already exists - looking it up instead.'
        request = self._service.projects().locations().registries().get(
            name=self.full_name)
        request.execute()
      else:
        raise
  ...
  def list_devices(self):
    ...
    request = self._service.projects().locations().registries().devices().list(
  ...
  def create_device_with_rs256(self, device_id, certificate_file):
    """Create a new device with the given id, using RS256 for authentication."""
  ...
  def patch_es256_for_auth(self, device_id, public_key_file):
    ...
    # Patch requests use a FieldMask to determine which fields to update. In
    # this case, we're updating the device's credentials with a new entry.
    request = self._service.projects().locations().registries().devices(
...
def parse_command_line_args():
  ...
  parser.add_argument(
      '--registry_id',
      default=None,
      help='Registry id. If not set a unique registry name will be generated.')
...
def main():
  ...
  device_registry = DeviceRegistry(args.project_id, registry_id,
                                   args.cloud_region, args.service_account_json,
                                   args.api_key, args.pubsub_topic)
  ...
  # important that you use unique public/private key pairs for each device (do
  # not reuse a key pair for multiple devices). This way if a private key is
  # compromised, only a single device will be affected.
  rs256_device_id = 'rs256-device'
  print 'Creating RS256 authenticated device', rs256_device_id
  device_registry.create_device_with_rs256(rs256_device_id,
                                           args.rsa_certificate_file)

  # Create an ES256 authenticated device. To demonstrate updating a device, we
  # will create the device with no authentication, and then update it to use
  # ES256 for authentication. Note that while one can create a device without
  # authentication, the MQTT client will not be able to connect to it.
  ...
  device_registry.patch_es256_for_auth(es256_device_id, args.ec_public_key_file)
